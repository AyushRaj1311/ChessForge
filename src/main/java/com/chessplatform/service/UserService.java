...
public class UserService implements UserDetailsService {
    ...
    @Transactional
    public void updateRatingAfterGame(User winner, User loser, boolean isDraw) {
        int kFactor = 32;
        if (isDraw) {
            int ratingChange = calculateEloChange(winner.getRating(), loser.getRating(), 0.5, kFactor);
            ...
            int winnerGain = calculateEloChange(winner.getRating(), loser.getRating(), 1.0, kFactor);
            winner.setRating(Math.max(100, winner.getRating() + winnerGain));
            loser.setRating(Math.max(100, loser.getRating() - winnerGain));
