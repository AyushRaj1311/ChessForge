...
public class MinimaxAI {
    ...
    public ChessMove getBestMove(Board board, PieceColor aiColor) {
        return getBestMove(board, aiColor, defaultMaxDepth);
    }

    public ChessMove getBestMove(Board board, PieceColor aiColor, int maxDepth) {
        ...
        for (ChessMove move : moves) {
            ...
            int score = minimax(board, maxDepth - 1, Integer.MIN_VALUE, Integer.MAX_VALUE, aiColor == PieceColor.BLACK, maxDepth);
    ...
    private int minimax(Board board, int depth, int alpha, int beta, boolean isMaximizing, int maxDepth) {
        ...
        if (isMaximizing) {
            ...
            for (ChessMove move : moves) {
                ...
                int eval = minimax(board, depth - 1, alpha, beta, false, maxDepth);
            ...
            for (ChessMove move : moves) {
                ...
                board.makeMove(move); // Apply directly!
                int eval = minimax(board, depth - 1, alpha, beta, true, maxDepth);
                board.unmakeMove();   // Revert immediately!
