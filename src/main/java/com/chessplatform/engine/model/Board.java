...
public class Board implements Serializable {
            ...
            for (char c : rankStr.toCharArray()) {
                if (Character.isDigit(c)) {
                    ...
                    if (p.getType() == PieceType.KING) {
                        if (p.getColor() == PieceColor.WHITE) board.whiteKingPos = Position.get(rank, col);
                        else board.blackKingPos = Position.get(rank, col);
                    }
                    col++;
                }
            }
    ...
    public void makeMove(ChessMove move) {
        ...
        if (move.isEnPassant()) {
            ...
            Position epPos = Position.get(capturedPawnRow, to.getCol());
        ...
        if (move.isCastle()) {
            ...
            if (to.getCol() == 6) { // Kingside
                setPiece(Position.get(rank, 5), getPiece(rank, 7));
                removePiece(Position.get(rank, 7));
            } else { // Queenside
                setPiece(Position.get(rank, 3), getPiece(rank, 0));
                removePiece(Position.get(rank, 0));
            }
        }
        ...
        if (movingPiece.getType() == PieceType.PAWN && Math.abs(to.getRow() - from.getRow()) == 2) {
            enPassantTarget = Position.get((from.getRow() + to.getRow()) / 2, from.getCol());
        }
    ...
    public void unmakeMove() {
        ...
        if (state.capturedPiece != null) {
            if (move.isEnPassant()) {
                ...
                setPiece(Position.get(capturedPawnRow, to.getCol()), state.capturedPiece);
            } else {
        ...
        if (move.isCastle()) {
            ...
            if (to.getCol() == 6) { // Kingside
                ...
                setPiece(Position.get(rank, 7), rook);
                removePiece(Position.get(rank, 5));
            } else { // Queenside
                Piece rook = getPiece(rank, 3);
                setPiece(Position.get(rank, 0), rook);
                removePiece(Position.get(rank, 3));
            }
        }
    }

    private void updateCastlingRights(Position pos) {
        if (pos.equals(Position.get(0, 4))) { whiteKingsideCastle = false; whiteQueensideCastle = false; }
        if (pos.equals(Position.get(7, 4))) { blackKingsideCastle = false; blackQueensideCastle = false; }
        if (pos.equals(Position.get(0, 0))) whiteQueensideCastle = false;
        if (pos.equals(Position.get(0, 7))) whiteKingsideCastle = false;
        if (pos.equals(Position.get(7, 0))) blackQueensideCastle = false;
        if (pos.equals(Position.get(7, 7))) blackKingsideCastle = false;
    }
